<template>
  <FaDrawer v-model="isShow" title="偏好设置" :side="settingsStore.settings.app.direction === 'ltr' ? 'right' : 'left'" :destroy-on-close="false" content-class="sm:min-w-md">
    <div
      v-if="
    ...
    <div v-if="settingsStore.mode === 'pc' && userStore.preferences.menu?.hasOwnProperty('mode')">
      ...
      <div v-if="userStore.preferences.menu?.hasOwnProperty('enableDark')" class="setting-item">
        <div class="label">
          暗黑导航栏
        ...
        <FaSwitch v-model="userStore.preferences.menu.enableDark" :disabled="settingsStore.currentColorScheme !== 'light'" />
      </div>
    </div>
      ...
      <div v-if="userStore.preferences.layout?.hasOwnProperty('widthModeScope')" class="setting-item">
        ...
        <div class="flex-center-start gap-1">
          <FaButton
            ...
            ]" :key="index" :variant="userStore.preferences.layout.widthModeScope === item.value ? 'default' : 'outline'" size="sm" :disabled="userStore.preferences.layout.widthMode !== 'center'" @click="userStore.preferences.layout?.hasOwnProperty('widthModeScope') && (userStore.preferences.layout.widthModeScope = (item.value as any))"
          >
            {{ item.label }}
          </FaButton>
        </div>
